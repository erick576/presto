        ...
        implements QueryRunner
{
    ...
    public DistributedQueryRunner(Session defaultSession, int nodeCount, Map<String, String> extraProperties)
            throws Exception
    {
        this(false, false, defaultSession, nodeCount, 1, extraProperties, ImmutableMap.of(), ImmutableMap.of(), DEFAULT_SQL_PARSER_OPTIONS, ENVIRONMENT, Optional.empty(), Optional.empty(), ImmutableList.of());
    }
        ...
        try {
            ...
            if (externalWorkerLauncher.isPresent()) {
                ...
                for (int i = (coordinatorCount + (resourceManagerEnabled ? 1 : 0)); i < nodeCount; i++) {
                    TestingPrestoServer worker = closer.register(createTestingPrestoServer(discoveryUrl, false, resourceManagerEnabled, false, catalogServerEnabled, false, extraProperties, parserOptions, environment, baseDataDir, extraModules));
                    servers.add(worker);
                }
            }
            ...
            if (resourceManagerEnabled) {
                resourceManager = Optional.of(closer.register(createTestingPrestoServer(discoveryUrl, true, true, false, false, false, resourceManagerProperties, parserOptions, environment, baseDataDir, extraModules)));
                servers.add(resourceManager.get());
            }

            if (catalogServerEnabled) {
                catalogServer = Optional.of(closer.register(createTestingPrestoServer(discoveryUrl, false, false, true, true, false, extraCoordinatorProperties, parserOptions, environment, baseDataDir, extraModules)));
                servers.add(catalogServer.get());
            }

            for (int i = 0; i < coordinatorCount; i++) {
                TestingPrestoServer coordinator = closer.register(createTestingPrestoServer(discoveryUrl, false, resourceManagerEnabled, false, catalogServerEnabled, true, extraCoordinatorProperties, parserOptions, environment, baseDataDir, extraModules));
    ...
    private static TestingPrestoServer createTestingPrestoServer(URI discoveryUri, boolean resourceManager, boolean resourceManagerEnabled, boolean catalogServer, boolean catalogServerEnabled, boolean coordinator, Map<String, String> extraProperties, SqlParserOptions parserOptions, String environment, Optional<Path> baseDataDir, List<Module> extraModules)
            throws Exception
    {
        long start = nanoTime();
        ...
        TestingPrestoServer server = new TestingPrestoServer(resourceManager, resourceManagerEnabled, catalogServer, catalogServerEnabled, coordinator, properties, environment, discoveryUri, parserOptions, extraModules, baseDataDir);
    ...
    public static class Builder
    {
        ...
        public DistributedQueryRunner build()
                throws Exception
        {
            return new DistributedQueryRunner(resourceManagerEnabled, catalogServerEnabled, defaultSession, nodeCount, coordinatorCount, extraProperties, coordinatorProperties, resourceManagerProperties, parserOptions, environment, baseDataDir, externalWorkerLauncher, extraModules);
        }
    }
}
